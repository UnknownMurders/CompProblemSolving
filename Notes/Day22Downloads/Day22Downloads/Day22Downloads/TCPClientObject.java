 ...
 * @version 9-16-2017
 */
public class TCPClientObject extends Application implements EventHandler<ActionEvent> {
   // Window Attributes
   private Stage stage;
   private Scene scene;
   private VBox root;
   ...
   private void doSend() {
      ...
      try {
         ...
         // send the file length to the server and flush the stream
         ...
         in.readLong(); // receive the file length back from the server
         
         taLog.appendText("Sending file\n");
         
         // send the File object to the server and flush the stream
         out.writeObject(myFile);
         out.flush();
         
         taLog.appendText("Done sending file\n");
         in.readUTF(); // receive "DONE" from the server
         taLog.appendText("Server has received the file\n");
         out.flush();
         
         // create a NameTag object and send it to the server, then wait for a "DONE" message
         NameTag tag = new NameTag("Chris","Russo","New Canaan","CT","MadDog@siriusxm.com");
         taLog.appendText("Sending NameTag with contents:\n" + tag + "\n");
         
         out.writeObject(tag);
         out.flush();
         
         in.readUTF();
         taLog.appendText("Server has received the NameTag\n");
      }
      catch(Exception e) {
